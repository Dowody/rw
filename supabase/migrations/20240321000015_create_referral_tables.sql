...
USING (
    auth.uid() IN (
        SELECT auth_id FROM public.users WHERE id = referrer_id
        UNION
        SELECT auth_id FROM public.users WHERE id = referred_id
    )
);
...
FOR INSERT
WITH CHECK (
    auth.uid() IN (SELECT auth_id FROM public.users WHERE id = referrer_id)
);
